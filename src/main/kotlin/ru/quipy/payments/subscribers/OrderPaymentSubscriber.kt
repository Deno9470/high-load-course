                ...
                paymentExecutor.submit {
                    ...
                    while (true) {
                        ...
                        if (currTime + accountInfo.getAverageDuration() - event.createdAt >= 80_000) {
                            logger.warn("${currTime + accountInfo.getAverageDuration() - event.createdAt >= 80_000}")
                            return@submit
                        }
                    ...
                    logger.warn("PendingRequestsAmount: ${accountInfo.getPendingRequestsAmount()}, ParallelRequests: ${accountInfo.getParallelRequests()}")
                    logger.warn("LastSecondRequestsAmount: ${accountInfo.getLastSecondRequestsAmount()}, RateLimitPerSec: ${accountInfo.getRateLimitPerSec()}")

                    logger.warn("AccountName: ${accountInfo.getExternalServiceProperties().accountName}, AccountPriority: ${accountInfo.getExternalServiceProperties().priority}")
